...
export class OrderComponent implements OnInit, OnDestroy, AfterViewInit{
    ...
    ngOnInit(): void {
        ...
        this.orderHeaderColumns = [
            ...
            {name: 'poCreDate', fieldName: 'poCreDate', type: 'data', width: '100', styleName: 'left-cell-text'
                , header: {text: '발주 생성일자' , styleName: 'left-cell-text'}
            },
            {name: 'poDate', fieldName: 'poDate', type: 'data', width: '100', styleName: 'left-cell-text'
                , header: {text: '발주 일자' , styleName: 'left-cell-text'}
            },
            ...
            {name: 'poAmt', fieldName: 'poAmt', type: 'number', width: '100', styleName: 'right-cell-text'
                , header: {text: '발주 금액' , styleName: 'left-cell-text'}
                , numberFormat : '#,##0'
            },
        ...
        this.gridList.onCellClicked = (grid, clickData) => {
            if(clickData.cellType === 'header'){
                if(clickData.cellType !== 'head'){
                    ...
                    this._orderService.getHeader(this.orderHeaderPagenation.page,this.orderHeaderPagenation.size,clickData.column,this.orderBy,this.searchForm.getRawValue());
                }
            }
            if(this.orderBy === 'asc'){
                this.orderBy = 'desc';
    ...
    searchSetValue(): void {
        ...
        if (this.searchForm.getRawValue().searchCondition2 === 'poNo') {
            this.searchForm.patchValue({'poNo': this.searchForm.getRawValue().searchText2});
        }
    ...
    pageEvent($event: PageEvent): void {
        ...
        this.searchSetValue();
        this._orderService.getHeader(this._paginator.pageIndex, this._paginator.pageSize, 'poNo', this.orderBy, this.searchForm.getRawValue());
    }
